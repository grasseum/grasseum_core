...
stream_exec.prototype.render_steam = function(after_load_queue){
    ...
    if(main.list_dir_config.length > 0 ){
        ...
        var var_list = main.list_dir_config[0];
          main.read_stream=main.cls_initialize.streamReader(var_list['file_src']);
          main.create_stream=main.cls_initialize.streamWrite(var_list['file_dest'],var_list['file_src']);
          
          main.local_stream_pipe = undefined;
          if(compt.has(main.cls_value_pipe,var_list['file_src']['ext'])){
            main.local_stream_pipe = main.cls_value_pipe[var_list['file_src']['ext']];

          }else{
            if(compt.has(main.cls_value_pipe,"__any__")){
                main.local_stream_pipe = main.cls_value_pipe["__any__"];
              }
          }


                main.cls_initialize.streamTransfom(main.read_stream,main.create_stream,main.local_stream_pipe,main.cls_require_pipe,main,after_load_queue)
...
stream_exec.prototype.timer_trigger_acton = function(internal_reference_value,after_load_queue){
    ...
    if(main.list_dir_config.length == main.count_list_dir && main.count_list_dir >0){
